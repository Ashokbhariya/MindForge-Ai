...
export default function QuizPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // FIX: Pass user_id in the submit payload so attempt stores it
      ...
      // FIX: Use api.getScore which uses the correct URL /api/score/{id}
      const scoreRes = await api.getScore(attemptId, userId);
      setScoreData(scoreRes);
      setShowAnswers(true);

      if ((scoreRes?.percentage ?? 100) < 75) {
        navigate("/confusion-detector", {
          state: {
            weakTopics: scoreRes?.weak_topics || [],
            score: scoreRes?.score ?? 0,
            total: scoreRes?.total ?? 0,
            percentage: scoreRes?.percentage ?? 0,
          },
        });
      }
    } catch (error) {
      ...
      {quizData.map((q, idx) => (
        <div key={q.id || idx} className="mb-4 p-3 border rounded bg-white shadow">
          ...
          {(q.options || []).map((opt, i) => {
            ...
            return (
              <label key={i} className={`block mb-1 p-2 rounded cursor-pointer ${isCorrect ? "bg-green-200" : isWrong ? "bg-red-200" : isSelected ? "bg-blue-100" : "hover:bg-gray-50"}`}>
                <input type="radio" name={`q-${idx}`} value={opt} className="mr-2"
                  onChange={() => handleOptionSelect(idx, opt)} checked={isSelected} disabled={showAnswers} />
                {opt}
              </label>
            );
          })}
      ...
      {scoreData && (
        <div className="mt-6 p-4 bg-purple-50 rounded border border-purple-200">
          ...
          {(scoreData.weak_topics || []).length > 0 && (
            <p className="text-sm text-gray-600 mt-1">Weak areas: {scoreData.weak_topics.join(", ")}</p>
          )}
        </div>
      )}
    </div>
