  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      try {
        ...
        if (qRes.ok) { const d = await qRes.json(); setQuizResults(Array.isArray(d) ? d : []); }

        const cRes = await fetch(`${BASE_URL}/api/quiz/confusion-topics?user_id=${userId}`);
        if (cRes.ok) { const d = await cRes.json(); if (Array.isArray(d)) setWeakTopics(d.map((s) => s.topic || "").filter(Boolean)); }

        const rRes = await fetch(`${BASE_URL}/api/roadmap/user/${userId}/latest`);
        if (rRes.ok) { const d = await rRes.json(); if (d?.topic) setLatestRoadmap(d); }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col font-inter">
      <div className="flex flex-1">
        ...
        {/* Sidebar — using navigate() instead of <a href> */}
        <aside className="w-64 bg-secondary text-white flex flex-col py-8 px-6 space-y-6">
          ...
          <nav className="flex flex-col space-y-4">
            <button onClick={() => navigate("/roadmap")}            className="text-white hover:text-accent text-left">Roadmap</button>
            <button onClick={() => navigate("/learn&quiz")}         className="text-white hover:text-accent text-left">Learn &amp; Quiz</button>
            <button onClick={() => navigate("/recallcard")}         className="text-white hover:text-accent text-left">Recall Card</button>
            <button onClick={() => navigate("/confusion-detector")} className="text-white hover:text-accent text-left">Confusion Detector</button>
            <button onClick={() => navigate("/progresscard")}       className="text-white hover:text-accent text-left">Progress Card</button>
            <button onClick={handleLogout}                          className="text-white hover:text-accent text-left">Logout</button>
          </nav>
        </aside>
            ...
            <>
              {user?.career_goal && (
                <div className="bg-gradient-to-r from-blue-700 to-indigo-500 text-white p-6 rounded-xl shadow mb-8 flex items-center gap-5">
                  ...
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-widest opacity-75 mb-1">Your Career Goal</p>
                    <p className="text-2xl font-bold">{user.career_goal}</p>
                    <p className="text-sm opacity-75 mt-1">Every roadmap and quiz is bringing you closer to this.</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                <div className="bg-[#1a213d] text-white p-6 rounded-lg md:col-span-2">
                  <h2 className="text-lg mb-2">{latestRoadmap ? "Continue Learning" : "Start Learning"}</h2>
                  ...
                  <div className="flex flex-col items-center">
                    <div className="relative w-24 h-24">
                      <svg className="w-full h-full" viewBox="0 0 36 36">
                        <path strokeWidth="3.8" fill="none" stroke="#e5e7eb" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                        <path strokeWidth="3.8" fill="none" stroke="#2563eb" strokeDasharray={`${avgScore}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                      </svg>
                      <span className="absolute inset-0 flex items-center justify-center text-lg font-semibold text-blue-600">{avgScore}%</span>
                    </div>
                    <p className="text-sm text-gray-500 mt-2">{quizResults.length} quiz{quizResults.length !== 1 ? "zes" : ""} taken</p>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow mb-10">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {uniqueWeak.map((topic, i) => (
                      <span key={i} className="bg-orange-100 text-orange-700 px-3 py-1 rounded-full text-sm font-medium">{topic}</span>
                    ))}
                  </div>
                </div>
              )}
                        ...
                        {[...quizResults].reverse().slice(0, 5).map((r, i) => {
                          ...
                          return (
                            <tr key={i} className="border-b last:border-none">
                              ...
                              <td className="py-2 text-gray-400 text-xs">{r.timestamp ? new Date(r.timestamp).toLocaleDateString() : "—"}</td>
                            </tr>
                          );
                        })}
                      </tbody>
              ...
              {quizResults.length === 0 && !latestRoadmap && (
                <div className="bg-white p-10 rounded-lg shadow text-center text-gray-500">
                  ...
                  <button onClick={() => navigate("/roadmap")} className="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700">
                    Generate a Roadmap
                  </button>
                </div>
              )}
