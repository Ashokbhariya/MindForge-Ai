...
def init_user_roadmaps_collection():
    """
    ✅ Only create if it doesn't exist — never delete on startup.
    Uses a real vector config so Qdrant doesn't panic on search.
    """
    try:
...
def insert_user_roadmap(user_id: str, roadmap_id: str, prompt: str):
    """
    ✅ Now includes a real vector so the collection doesn't crash on search.
    The vector is the embedding of the prompt for semantic search later.
    """
    try:
...
def search_user_roadmaps(user_id: str, query: str, top_k: int = 5):
    """✅ Search user-specific roadmaps by semantic similarity."""
    try:
        query_vector = embedding_model.encode(query).tolist()
        results = qdrant_client.search(
            collection_name=USER_COLLECTION,
